...
class RTNative {
    ...
    init = () => {
        ...
        this.Utilities = new RTNativeUtilities(this.shadowRoot, this.content);

        /* Initialize a Node Manager */
        this.NodeManager = new RTNativeNodeManager(this.shadowRoot, this.content, this.Utilities);

        /* Initialize Action Options (e.g., cut, copy, paste) */
        this.ActionOptions = new RTNativeActionOptions(this.shadowRoot, this.content, this.Utilities);

        /* Initialize List Provider */
        this.ListProvider = new RTNativeListProvider(this.shadowRoot, this.content, this.Utilities, this.NodeManager);

        /* Initialize the color pickers */
        this.ColorPickers = {};
        const colorModal = "rich-text-box-text-color-modal";
        const bgColorModal = "rich-text-box-text-bg-color-modal";
        this.ColorPickers[colorModal] = new RTNativeColorDialog(this.shadowRoot, this.content, colorModal, this.NodeManager, this.Utilities);
        this.ColorPickers[bgColorModal] = new RTNativeColorDialog(this.shadowRoot, this.content, bgColorModal, this.NodeManager, this.Utilities);

        /* Initialize the Link Dialog */
        this.LinkDialog = new RTNativeLinkDialog(this.shadowRoot, this.content, this.Utilities);

        /* Initialize Image Dialog */
        this.ImageDialog = new RTNativeImageDialog(this.shadowRoot, this.content, this.Utilities);

        /* Initialize Upload Image Dialog - removed to prevent abuse. */
        this.UploadImageDialog = new RTNativeUploadImageDialog(this.shadowRoot, this.content, this.Utilities); 

        /* Initialize Blockquote Dialog */
        this.BlockQuoteDialog = new RTNativeBlockQuoteDialog(this.shadowRoot, this.content, this.Utilities);

        /* Initialize Code Block Dialog */
        this.CodeBlockDialog = new RTNativeCodeBlockDialog(this.shadowRoot, this.content, this.Utilities);

        /* Initialize the Media Dialog */
        this.MediaDialog = new RTNativeMediaDialog(this.shadowRoot, this.content, this.Utilities);

        /* Initialize the Video Dialog */
        this.VideoDialog = new RTNativeVideoDialog(this.shadowRoot, this.content, this.Utilities);

        /* Table Dialog */
        this.TableDialog = new RTNativeTableDialog(this.shadowRoot, this.content, this.Utilities);

        /* Create a state manager */
        this.StateManager = new RTNativeStateManager(this.content, this.source, this.Utilities, this.dotNetObjectReference, this.contentContainer);
    ...
    hydrateShadowTree = () => {
        ...
        this.findBar.className = 'rtb-find-bar';
        this.findBar.setAttribute('role', 'search');
        this.findBar.setAttribute('aria-label', 'Find in editor');
        ...
        this._setupToolbarOverflow(toolbar);
        ...
        this.contextMenu.className = 'rtb-context-menu';
    ...
    _setupToolbarOverflow = (toolbar) => {
        // IDs of buttons whose wrapper divs must always stay pinned on the toolbar
        const PINNED_IDS = new Set([
            'blazing-rich-text-file-button',
            'blazing-rich-text-font-button',
            'blazing-rich-text-size-button',
            'blazing-rich-text-format-button',
            'blazing-rich-text-insert-button'
        ]);
        const isPinned = (el) => {
            if (!el.classList.contains('rich-text-box-dropdown')) return false;
            const inner = el.querySelector(':scope > button');
            return inner && PINNED_IDS.has(inner.id);
        };

        // Classify every current toolbar child
        const pinned = [], movable = [], dialogs = [];
        for (const child of Array.from(toolbar.children)) {
            if (child.tagName === 'DIALOG') dialogs.push(child);
            else if (isPinned(child))        pinned.push(child);
            else                             movable.push(child);
        }

        // Items container holds only movable items so overflow:hidden is safe
        const itemsContainer = document.createElement('div');
        itemsContainer.className = 'rtb-toolbar-items';
        for (const child of movable) itemsContainer.appendChild(child);

        // Rebuild toolbar: [pinned] [items container] [more btn] [dropdown] [dialogs]
        while (toolbar.firstChild) toolbar.removeChild(toolbar.firstChild);
        for (const child of pinned) toolbar.appendChild(child);
        toolbar.appendChild(itemsContainer);

        // Create the "More" button (three-dot icon)
        const overflowBtn = document.createElement('button');
        overflowBtn.className = 'rtb-overflow-btn';
        overflowBtn.setAttribute('aria-label', 'More options');
        overflowBtn.setAttribute('title', 'More options');
        overflowBtn.setAttribute('aria-expanded', 'false');
        overflowBtn.setAttribute('aria-haspopup', 'true');
        overflowBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M480-160q-33 0-56.5-23.5T400-240q0-33 23.5-56.5T480-320q33 0 56.5 23.5T560-240q0 33-23.5 56.5T480-160Zm0-240q-33 0-56.5-23.5T400-480q0-33 23.5-56.5T480-560q33 0 56.5 23.5T560-480q0 33-23.5 56.5T480-400Zm0-240q-33 0-56.5-23.5T400-720q0-33 23.5-56.5T480-800q33 0 56.5 23.5T560-720q0 33-23.5 56.5T480-640Z"/></svg>`;
        // Create the overflow dropdown panel
        const dropdown = document.createElement('div');
        dropdown.className = 'rtb-overflow-dropdown';
        dropdown.setAttribute('role', 'toolbar');
        dropdown.setAttribute('aria-label', 'More toolbar options');

        // Wrap button and dropdown so the dropdown anchors to the button via CSS
        const overflowWrapper = document.createElement('div');
        overflowWrapper.className = 'rtb-overflow-wrapper';
        overflowWrapper.style.display = 'none';
        overflowWrapper.appendChild(overflowBtn);
        overflowWrapper.appendChild(dropdown);
        toolbar.appendChild(overflowWrapper);

        // Toggle dropdown when More button is clicked
        overflowBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const isOpen = dropdown.classList.toggle('rtb-overflow-open');
            overflowBtn.setAttribute('aria-expanded', String(isOpen));
            if (isOpen) {
                dropdown.style.maxWidth = (toolbar.offsetWidth - 10) + 'px';
            }
        });

        // Close dropdown when any item inside it is clicked
        dropdown.addEventListener('click', () => {
            dropdown.classList.remove('rtb-overflow-open');
            overflowBtn.setAttribute('aria-expanded', 'false');
        });
        ...
        this._overflowBtn = overflowBtn;
        this._overflowDropdown = dropdown;
        this._overflowWrapper = overflowWrapper;
        this._overflowItemsContainer = itemsContainer;
        this._overflowToolbar = toolbar;

        // Recalculate overflow whenever the toolbar or the items container changes width.
        // The items container shrinks when pinned buttons (Font/Format/Size) grow wider,
        // so observing it catches label-text changes without touching clearSettings.
        const ro = new ResizeObserver(() => this._scheduleOverflowUpdate());
        ro.observe(toolbar);
        ro.observe(itemsContainer);
        this._overflowResizeObserver = ro;

        // Dialogs stay as direct toolbar children so they position against the viewport
        for (const child of dialogs) toolbar.appendChild(child);
    }

    _scheduleOverflowUpdate = () => {
        if (!this._overflowItemsContainer || this._overflowUpdatePending) return;
    ...
    _updateToolbarOverflow = () => {
        const items = this._overflowItemsContainer;
        const wrapper = this._overflowWrapper;
        const dropdown = this._overflowDropdown;
        const toolbar = this._overflowToolbar;
        if (!items) return;

        if (dropdown.classList.contains('rtb-overflow-open')) {
            dropdown.style.maxWidth = (toolbar.offsetWidth - 10) + 'px';
        }

        // Restore all overflowed items back to the items container first
        Array.from(dropdown.children).forEach(el => items.appendChild(el));
        wrapper.style.display = 'none';

        // If everything fits, we're done
        if (items.scrollWidth <= items.offsetWidth) return;

        // Items overflow — show the More button (narrows items container)
        wrapper.style.display = '';

        // Move items from the right until everything fits
        const children = Array.from(items.children);
        for (let i = children.length - 1; i >= 0; i--) {
            if (items.scrollWidth <= items.offsetWidth) break;
            dropdown.insertBefore(children[i], dropdown.firstChild);
        }

        if (!dropdown.firstChild) wrapper.style.display = 'none';
    }
    ...
    addEventListeners = () => {
        ...
        this.content.addEventListener('dblclick', (event) => {
            switch (event.target.tagName) {
                ...
                case 'BLOCKQUOTE':
                    event.preventDefault();
                    this.openBlockQuoteDialogForElement(event.target);
                    break;
                case 'PRE':
                    event.preventDefault();
                    this.openCodeBlockDialogForElement(event.target);
                    break;
                case 'CODE': {
                    const pre = event.target.closest('pre');
                    if (pre && this.content.contains(pre)) {
                        event.preventDefault();
                        this.openCodeBlockDialogForElement(pre);
                    }
                    break;
                }
                default: {
                    ...
                    const blockquote = event.target.closest('blockquote');
                    if (blockquote && this.content.contains(blockquote)) {
                        event.preventDefault();
                        this.openBlockQuoteDialogForElement(blockquote);
                    }
                    break;
                }
            }
        ...
        this.source.addEventListener('keydown', (event) => {
            ...
            if (event.altKey && !event.ctrlKey && event.key === 'l') {
                event.preventDefault();
                this.dropdown('blazing-rich-text-file-button-dropdown');
            }
            if (event.altKey && !event.ctrlKey && event.key === 'f') {
                event.preventDefault();
                this.dropdown('blazing-rich-text-font-button-dropdown');
            }
            if (event.altKey && !event.ctrlKey && event.key === 's') {
                event.preventDefault();
                this.dropdown('blazing-rich-text-size-button-dropdown');
            }
            if (event.altKey && !event.ctrlKey && event.key === 't') {
                event.preventDefault();
                this.dropdown('blazing-rich-text-format-button-dropdown');
            }
            if (event.altKey && !event.ctrlKey && event.key === 'i') {
                event.preventDefault();
                this.dropdown('blazing-rich-text-insert-button-dropdown');
            }
        ...
        /* Find bar wiring */
        const findInput = this.findBar.querySelector('.rtb-find-input');
        const findCount = this.findBar.querySelector('.rtb-find-count');
        const [prevBtn, nextBtn] = this.findBar.querySelectorAll('.rtb-find-nav-btn');
        const closeBtn = this.findBar.querySelector('.rtb-find-close-btn');
        ...
        findInput.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Escape') { e.preventDefault(); this.closeFindBar(); }
        });
        ...
        /* Context menu */
        this.content.addEventListener('contextmenu', (event) => this.openEditorContextMenu(event));
    ...
    keyEvents = (event) => {
        ...
        if (event.key === 'Enter' && !event.ctrlKey && !event.altKey && !event.metaKey) {
            if (this.TableDialog.addRowFromLastCell()) {
                event.preventDefault();
                return;
            }
            if (this.insertParagraphAfterStructuredBlock()) {
                event.preventDefault();
                return;
            }
        }
    ...
    getStructuredBlockFromNode = (node) => {
        if (!node || node === this.content) { return null; }
        const element = node.nodeType === Node.ELEMENT_NODE ? node : node.parentElement;
        if (!element || !this.content.contains(element)) { return null; }

        const codeBlock = element.closest('pre');
        if (codeBlock && this.content.contains(codeBlock)) {
            return codeBlock;
        }
    ...
    insertParagraphAfterStructuredBlock = () => {
        const selection = this.Utilities.getSelection();
        if (!selection || selection.rangeCount === 0) { return false; }

        const block = this.getStructuredBlockFromNode(selection.anchorNode);
    ...
    loadPreviewWindow = (previewWindow) => {
        ...
        const isDark = hostEl && (hostEl.classList.contains('dark') || hostEl.classList.contains('fluent-dark'));
        const baseStyle = [
            '<style>',
            `html{margin:0;color-scheme:${isDark ? 'dark' : 'light'};}`,
            'body{margin:0;}',
            'body{',
            `  padding:20px 24px;`,
            `  color:${getVar('--rtb-content-text', '#242424')};`,
            `  font-size:${getVar('--rtb-content-size', '16px')};`,
            `  font-family:${getVar('--rtb-content-font', 'Arial, Helvetica, Verdana, sans-serif')};`,
            `  line-height:${getVar('--rtb-content-line-height', '1.55')};`,
            ...
            `blockquote{` +
                ...
                `color:${getVar('--rtb-quote-text', getVar('--rtb-content-text', '#242424'))};` +
        ...
        iframe.srcdoc = [
            ...
            '</head><body>',
            html,
            '<script>document.addEventListener("click",function(e){var a=e.target.closest("a");if(a&&a.href){e.preventDefault();window.open(a.href,"_blank","noopener,noreferrer");}});<\/script>',
            '</body></html>'
        ].join('');
    ...
    toggleStatusBar = () => {
        ...
        const menuLabel = this.shadowRoot.getElementById('blazing-rich-text-file-statusbar-label');
        if (menuLabel) menuLabel.textContent = isHidden ? 'Hide Status Bar' : 'Show Status Bar';
    }
    ...
    openFindBar = () => {
        ...
        const input = this.findBar.querySelector('.rtb-find-input');
    ...
    closeFindBar = () => {
        ...
        this._findMatches = [];
        this._findIndex = -1;
        this.findBar.querySelector('.rtb-find-count').textContent = '';
        if (this.EditMode) {
            this.content.focus();
        } else {
            this.source.focus();
    ...
    openHelpDialog = () => {
        this.shadowRoot.getElementById('rich-text-box-help-modal').showModal();
    }

    fileMenuAction = (action) => {
        this.closeDropdown('blazing-rich-text-file-button-dropdown');
        switch (action) {
            case 'save':          this.saveHtml();       break;
            case 'find':          this.openFindBar();    break;
            case 'toggleStatusBar': this.toggleStatusBar(); break;
            case 'shortcuts':     this.openHelpDialog(); break;
        }
    }

    closeHelpDialog = () => {
        this.shadowRoot.getElementById('rich-text-box-help-modal').close();
    }

    openEditorContextMenu = (event) => {
        if (!this.EditMode) return;
        const target = event.target instanceof Element ? event.target : event.target?.parentElement;
        if (!(target instanceof Element) || !this.content.contains(target)) return;

        const link       = target.closest('a');
        const img        = target.closest('img');
        const video      = target.tagName === 'SOURCE' ? target.closest('video') : target.closest('video');
        const pre        = target.closest('pre');
        const blockquote = target.closest('blockquote');
        const mediaObj   = target.closest('object');
        const table      = target.closest('table');
        const ul         = target.closest('ul');
        const ol         = target.closest('ol');
        const hr         = target.closest('hr') || (() => {
            for (const h of this.content.querySelectorAll('hr')) {
                const r = h.getBoundingClientRect();
                if (event.clientX >= r.left && event.clientX <= r.right &&
                    event.clientY >= r.top - 10 && event.clientY <= r.bottom + 10) return h;
            }
            return null;
        })();

        let items;
        if (link && this.content.contains(link))             items = this._buildLinkContextItems(link);
        else if (img && this.content.contains(img))          items = this._buildImageContextItems(img);
        else if (video && this.content.contains(video))      items = this._buildVideoContextItems(video);
        else if (pre && this.content.contains(pre))          items = this._buildCodeBlockContextItems(pre);
        else if (blockquote && this.content.contains(blockquote)) items = this._buildBlockQuoteContextItems(blockquote);
        else if (mediaObj && this.content.contains(mediaObj)) items = this._buildMediaContextItems(mediaObj);
        else if (table && this.content.contains(table))      items = this._buildTableContextItems(table);
        else if (hr && this.content.contains(hr))            items = this._buildHrContextItems(hr);
        else if (ul && this.content.contains(ul))            items = this._buildBulletListContextItems(ul, target);
        else if (ol && this.content.contains(ol))            items = this._buildNumberListContextItems(ol, target);
        else                                                  items = this._buildTextContextItems(target);
    ...
    _buildTextContextItems = (target) => {
        const items = [];

        const boldEl        = this._getInlineFormatAtNode(target, 'bold');
        const italicEl      = this._getInlineFormatAtNode(target, 'italic');
        const underlineEl   = this._getInlineFormatAtNode(target, 'underline');
        const strikeEl      = this._getInlineFormatAtNode(target, 'line-through');
        const subscriptEl   = this._getInlineFormatAtNode(target, 'subscript');
        const superscriptEl = this._getInlineFormatAtNode(target, 'superscript');
        const codeEl        = this._getInlineFormatAtNode(target, 'code');

        let hasColor = false;
        let colorEl = target instanceof Element ? target : target?.parentElement;
        while (colorEl && colorEl !== this.content && this.content.contains(colorEl)) {
            if (colorEl.style.color || colorEl.style.backgroundColor) { hasColor = true; break; }
            colorEl = colorEl.parentElement;
        }

        if (boldEl)        items.push({ label: 'Remove Bold',          icon: 'Delete',           action: () => { this.content.focus(); this.bold(); } });
        if (italicEl)      items.push({ label: 'Remove Italic',        icon: 'Delete',           action: () => { this.content.focus(); this.italic(); } });
        if (underlineEl)   items.push({ label: 'Remove Underline',     icon: 'Delete',           action: () => { this.content.focus(); this.underline(); } });
        if (strikeEl)      items.push({ label: 'Remove Strikethrough', icon: 'Delete',           action: () => { this.content.focus(); this.strikethrough(); } });
        if (subscriptEl)   items.push({ label: 'Remove Subscript',     icon: 'Delete',           action: () => { this.content.focus(); this.subscript(); } });
        if (superscriptEl) items.push({ label: 'Remove Superscript',   icon: 'Delete',           action: () => { this.content.focus(); this.superscript(); } });
        if (codeEl)        items.push({ label: 'Remove Inline Code',   icon: 'Delete',           action: () => { this.content.focus(); this.inlinecode(); } });
        if (hasColor)      items.push({ label: 'Remove Color',         icon: 'RemoveTextFormat', action: () => this.removeTextColor() });

        if (items.length > 0) items.push({ divider: true });

        [
            ['None',      'none'], ['Paragraph', 'p'],
            ['Heading 1', 'h1'],   ['Heading 2',  'h2'],
            ['Heading 3', 'h3'],   ['Heading 4',  'h4'],
            ['Heading 5', 'h5'],   ['Heading 6',  'h6'],
        ].forEach(([label, fmt]) => {
            items.push({ label, icon: 'Format', action: () => { this.content.focus(); this.format(fmt); } });
        });

        return items;
    }

    _buildLinkContextItems = (link) => [
        { label: 'Open Link',   icon: 'OpenInNew',  action: () => window.open(link.href, '_blank', 'noopener,noreferrer') },
        { label: 'Edit Link',   icon: 'CreateLink', action: () => this._selectNodeForDialog(link, true,  () => this.openLinkDialog()) },
        { label: 'Remove Link', icon: 'RemoveLink', action: () => this._selectNodeForDialog(link, true,  () => this.removeLink()) },
    ]

    _buildImageContextItems = (img) => [
        {
            label: 'Edit Image', icon: 'Image',
            action: () => {
                const sel = this.Utilities.getSelection() || window.getSelection();
                const range = document.createRange();
                range.selectNode(img);
                sel.removeAllRanges();
                sel.addRange(range);
                if (!img.src?.startsWith('data')) this.openImageDialog();
                else this.uploadImageDialog();
            }
        },
        { label: 'Remove Image', icon: 'Delete', action: () => this._removeContentElement(img) },
    ]

    _buildVideoContextItems = (video) => [
        { label: 'Edit Video',   icon: 'VideoFile', action: () => this.openVideoDialogForElement(video) },
        { label: 'Remove Video', icon: 'Delete',    action: () => this._removeContentElement(video) },
    ]

    _buildBlockQuoteContextItems = (blockquote) => [
        { label: 'Edit Block Quote',   icon: 'Quote',  action: () => this.openBlockQuoteDialogForElement(blockquote) },
        { label: 'Remove Block Quote', icon: 'Delete', action: () => this._removeContentElement(blockquote) },
    ]

    _buildCodeBlockContextItems = (pre) => [
        { label: 'Edit Code Block',   icon: 'CodeBlocks', action: () => this.openCodeBlockDialogForElement(pre) },
        { label: 'Remove Code Block', icon: 'Delete',     action: () => this._removeContentElement(pre) },
    ]

    _buildMediaContextItems = (mediaObj) => [
        { label: 'Edit Media',   icon: 'PermMedia', action: () => this._selectNodeForDialog(mediaObj, false, () => this.openMediaDialog()) },
        { label: 'Remove Media', icon: 'Delete',    action: () => this._removeContentElement(mediaObj) },
    ]

    _buildTableContextItems = (table) => [
        {
            label: 'Edit Table', icon: 'Table',
            action: () => {
        ...
        { label: 'Remove Table', icon: 'Delete', action: () => this._removeContentElement(table) },
    ]

    _buildHrContextItems = (hr) => [{
        label: 'Remove Horizontal Rule', icon: 'Delete',
        action: () => this._removeContentElement(hr),
    }]

    _buildBulletListContextItems = (list, target) => {
        const items = [];
        const boldEl   = this._getInlineFormatAtNode(target, 'bold');
        const italicEl = this._getInlineFormatAtNode(target, 'italic');
        const underEl  = this._getInlineFormatAtNode(target, 'underline');
        const strikeEl = this._getInlineFormatAtNode(target, 'line-through');
        const codeEl   = this._getInlineFormatAtNode(target, 'code');
        if (boldEl)   items.push({ label: 'Remove Bold',          icon: 'Delete', action: () => { this.content.focus(); this.bold(); } });
        if (italicEl) items.push({ label: 'Remove Italic',        icon: 'Delete', action: () => { this.content.focus(); this.italic(); } });
        if (underEl)  items.push({ label: 'Remove Underline',     icon: 'Delete', action: () => { this.content.focus(); this.underline(); } });
        if (strikeEl) items.push({ label: 'Remove Strikethrough', icon: 'Delete', action: () => { this.content.focus(); this.strikethrough(); } });
        if (codeEl)   items.push({ label: 'Remove Inline Code',   icon: 'Delete', action: () => { this.content.focus(); this.inlinecode(); } });
        if (items.length > 0) items.push({ divider: true });
        items.push({
            label: 'Remove Bullet List', icon: 'Delete',
            action: () => {
                this.content.focus();
                const sel = this.Utilities.getSelection() || window.getSelection();
                const range = document.createRange();
                const firstLi = list.querySelector('li');
                if (firstLi) { range.setStart(firstLi, 0); } else { range.setStart(list, 0); }
                range.collapse(true);
                sel.removeAllRanges();
                sel.addRange(range);
                this.unorderedlist();
    ...
    _buildNumberListContextItems = (list, target) => {
        const items = [];
        const boldEl   = this._getInlineFormatAtNode(target, 'bold');
        const italicEl = this._getInlineFormatAtNode(target, 'italic');
        const underEl  = this._getInlineFormatAtNode(target, 'underline');
        ...
        if (items.length > 0) items.push({ divider: true });
        items.push({
            label: 'Remove Number List', icon: 'Delete',
            action: () => {
                this.content.focus();
                const sel = this.Utilities.getSelection() || window.getSelection();
                const range = document.createRange();
    ...
    _selectNodeForDialog = (element, contents, callback) => {
        if (!element || !this.content.contains(element)) return;
        const sel = this.Utilities.getSelection() || window.getSelection();
        const range = document.createRange();
        if (contents) range.selectNodeContents(element);
        else range.selectNode(element);
        sel.removeAllRanges();
        sel.addRange(range);
        callback();
    }

    _removeContentElement = (element) => {
        if (!element || !this.content.contains(element)) return;
        element.remove();
        this.NodeManager.refreshUI();
        this.content.dispatchEvent(new Event('input', { bubbles: true }));
    ...
    _showContextMenu = (event, items) => {
        ...
        this.contextMenu.replaceChildren(...items.map(item => {
            if (item.divider) {
                const d = document.createElement('div');
                d.className = 'rtb-context-menu-divider';
                return d;
            }
            return this._createContextMenuItem(item);
        }));
        ...
        const vw = vp?.width  || window.innerWidth  || document.documentElement.clientWidth;
        const vh = vp?.height || window.innerHeight || document.documentElement.clientHeight;
        const vl = vp?.offsetLeft || 0;
        const vt = vp?.offsetTop  || 0;
        const clampedX = Math.min(Math.max(vl + 4, event.clientX), vl + vw - r.width  - 4);
        const clampedY = Math.min(Math.max(vt + 4, event.clientY), vt + vh - r.height - 4);
        this.contextMenu.style.left = `${clampedX - r.left}px`;
        this.contextMenu.style.top  = `${clampedY - r.top}px`;
    }

    _createContextMenuItem = (item) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.setAttribute('role', 'menuitem');
        btn.innerHTML = `${this._contextMenuIcon(item.icon)}<span>${item.label}</span>`;
    ...
    _contextMenuIcon = (name) => {
        const extra = {
            Format:    'M280-160v-80h400v80H280Zm0-160v-80h400v80H280Zm-80-160v-320h560v320H200Zm80-80h400v-160H280v160Z',
            OpenInNew: 'M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h280v80H200v560h560v-280h80v280q0 33-23.5 56.5T760-120H200Zm188-212-56-56 372-372H560v-80h280v280h-80v-144L388-332Z',
        };
        const path = _RTB_ICONS[name] || extra[name] || '';
        if (!path) return '';
        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" aria-hidden="true" focusable="false"><path d="${path}"/></svg>`;
    }
    ...
    _getInlineFormatAtNode = (node, type) => {
        let el = node instanceof Element ? node : node?.parentElement;
        while (el && el !== this.content && this.content.contains(el)) {
            switch (type) {
                case 'bold':        if (el.style.fontWeight === 'bold') return el; break;
                case 'italic':      if (el.style.fontStyle  === 'italic') return el; break;
                case 'underline':   if (el.style.textDecoration?.includes('underline')) return el; break;
                case 'line-through': if (el.style.textDecoration?.includes('line-through')) return el; break;
                case 'subscript':   if (el.style.verticalAlign === 'sub') return el; break;
                case 'superscript': if (el.style.verticalAlign === 'super') return el; break;
                case 'code':        if (el.nodeName === 'CODE' && el.parentElement?.nodeName !== 'PRE') return el; break;
            }
            el = el.parentElement;
        }
        return null;
    ...
    findNext = () => {
        const query = this.findBar.querySelector('.rtb-find-input').value;
        if (!query) return;
        if (!this._findMatches.length) this._runSearch(query);
        if (!this._findMatches.length) return;
        this._findIndex = (this._findIndex + 1) % this._findMatches.length;
        this._applyFindMatch();
    }
    ...
    findPrev = () => {
        ...
        if (!this._findMatches.length) this._runSearch(query);
        if (!this._findMatches.length) return;
        this._findIndex = (this._findIndex <= 0 ? this._findMatches.length : this._findIndex) - 1;
        this._applyFindMatch();
    }
    ...
    _runSearch = (query) => {
        this._findMatches = [];
        this._findIndex = -1;
        const countEl = this.findBar.querySelector('.rtb-find-count');
        ...
        if (this.EditMode) {
            ...
            while ((node = walker.nextNode())) {
                ...
                while ((pos = text.indexOf(lower, pos)) !== -1) {
                    this._findMatches.push({ node, start: pos, end: pos + query.length });
                    pos++;
                }
            }
        } else {
            const text = this.source.value.toLowerCase();
            let pos = 0;
            while ((pos = text.indexOf(lower, pos)) !== -1) {
                this._findMatches.push({ start: pos, end: pos + query.length });
                pos++;
            }
        }
    ...
    _applyFindMatch = () => {
        ...
        if (this.EditMode) {
            ...
            const saved = this.source.value;
            this.source.value = saved.substring(0, match.start);
            const targetScrollTop = this.source.scrollHeight;
            this.source.value = saved;
            this.source.focus({ preventScroll: true });
            this.source.setSelectionRange(match.start, match.end);
            this.source.scrollTop = targetScrollTop <= this.source.clientHeight
                ? 0
                : Math.max(0, targetScrollTop - this.source.clientHeight / 2);
        }
    }
    ...
    updateWordCount = () => {
        ...
        this.statusBar.textContent =
            `${fmt(words)} word${words !== 1 ? 's' : ''} · ${fmt(chars)} character${chars !== 1 ? 's' : ''}`;
    }
    ...
    _syncWordCountVisibility = () => {
        ...
        const menuLabel = this.shadowRoot.getElementById('blazing-rich-text-file-statusbar-label');
        if (menuLabel) menuLabel.textContent = show ? 'Hide Status Bar' : 'Show Status Bar';
    }
    ...
    saveHtml = () => {
        ...
        a.href     = url;
        a.download = this.createTimestampedHtmlFileName();
        a.click();
        URL.revokeObjectURL(url);
    }
    createTimestampedHtmlFileName = () => {
        const now = new Date();
    ...
    decreaseIndent = () => {
        ...
        const list = this.ListProvider.getList(selection.anchorNode);
        if (list) {
            this.ListProvider.decreaseIndent(selection, list);
        }
        else {
    ...
    openBlockQuoteDialogForElement = (blockquote) => {
        if (!blockquote || blockquote.nodeName !== 'BLOCKQUOTE' || !this.content.contains(blockquote)) { return; }

        this.lockToolbar = true;
        const selection = this.Utilities.getSelection() || window.getSelection();
        const range = document.createRange();
        range.selectNodeContents(blockquote);
        selection.removeAllRanges();
        selection.addRange(range);
        this.savedSelection = this.Utilities.saveSelection(selection);
        this.BlockQuoteDialog.openBlockQuoteDialogForElement(blockquote, range);
    }
    insertBlockQuote = () => {
    ...
    openCodeBlockDialogForElement = (pre) => {
        if (!pre || pre.nodeName !== 'PRE' || !this.content.contains(pre)) { return; }
        ...
        selection.addRange(range);
        this.savedSelection = this.Utilities.saveSelection(selection);
        this.CodeBlockDialog.openCodeBlockDialogForElement(pre, range);
    }
    insertCodeBlock = () => {
    ...
    getHtml = async () => {
        ...
        this.disableButtons();
        if (this.findBar.style.display !== 'none') this._runSearch(this.findBar.querySelector('.rtb-find-input').value);
    };
    getCode = async () => {
    ...
    insertEmoji = (emoji) => {
        if (this.EditMode !== true || !emoji) return;
...
class RTNativeNodeManager {
    ...
    selectButtons = (el) => {
        ...
        const inlineCode = this.getButton("blazing-rich-text-inline-code-button");
        ...
        while (el !== this.content && el.parentNode !== null && this.content.contains(el.parentNode)) {
            ...
            if (el.parentNode.nodeName == "CODE") {
                if (el.parentNode.parentElement?.nodeName !== 'PRE') {
                    this._press(inlineCode);
                } else {
    ...
    closeDropdowns = () => {
        ...
        /* Close the overflow panel */
        const overflowPanel = this.shadowRoot.querySelector('.rtb-overflow-dropdown');
        if (overflowPanel && overflowPanel.classList.contains('rtb-overflow-open')) {
            overflowPanel.classList.remove('rtb-overflow-open');
            const overflowBtn = this.shadowRoot.querySelector('.rtb-overflow-btn');
...
class RTNativeListProvider {
            ...
            if (list != null) {
                ...
                if (selectedText.length === 0) {
                    ...
                    if (node.nodeType === Node.ELEMENT_NODE
                        ...
                        this.appendListItemContent(liElement, clonedContent);
    ...
    appendListItemContent = (liElement, content) => {
        if (!liElement || !content) { return; }

        if (this.isParagraphChild(content)) {
            while (content.firstChild) {
                liElement.appendChild(content.firstChild);
            }
            return;
        }
    ...
    removelist = (list) => {
        ...
        if (list.nodeName === "OL" || list.nodeName === "UL") {
            this.removeListAsParagraphs(list);
            return;
        }
    ...
    removeListAsParagraphs = (list) => {
        ...
        while (list.firstChild) {
            ...
            while (listItem.firstChild) {
                ...
                if (this.isParagraphChild(child)) {
    ...
    isParagraphChild = (node) => {
        if (!node || node.nodeType !== Node.ELEMENT_NODE) { return false; }
        return ["P", "DIV"].includes(node.nodeName);
    }
    increaseIndent = (selection, list) => {
        if (!selection || !list || (list.nodeName !== 'UL' && list.nodeName !== 'OL')) { return; }
        ...
        let previousItem = firstItem.previousElementSibling;

        if (!previousItem || previousItem.nodeName !== 'LI') { return; }

        let targetList = Array.from(previousItem.children)
            .find(child => child.nodeName === list.nodeName);

        if (!targetList) {
            targetList = document.createElement(list.nodeName);
            previousItem.appendChild(targetList);
        }
        ...
        this.selectListItemStart(selection, selectedNodes[0]);
    }

    decreaseIndent = (selection, list) => {
        if (!selection || !list || (list.nodeName !== 'UL' && list.nodeName !== 'OL')) {
            return;
        }

        const selectedNodes = this.getSelectedNodes(selection, list);
        if (!selectedNodes || selectedNodes.length === 0) { return; }

        const parentItem = list.parentElement;
        const parentList = parentItem ? parentItem.parentElement : null;

        if (!parentItem || parentItem.nodeName !== 'LI' || !parentList || (parentList.nodeName !== 'UL' && parentList.nodeName !== 'OL')) {
            return;
        }

        const lastSelectedNode = selectedNodes[selectedNodes.length - 1];
        const afterList = document.createElement(list.nodeName);
        while (lastSelectedNode.nextElementSibling) {
            afterList.appendChild(lastSelectedNode.nextElementSibling);
        }
        ...
        parentList.insertBefore(fragment, parentItem.nextSibling);

        if (afterList.children.length > 0) {
            lastSelectedNode.appendChild(afterList);
        }
    ...
    saveListSelection = (selection) => {
        if (!selection || selection.rangeCount === 0) { return null; }
        return selection.getRangeAt(0).cloneRange();
    }
    restoreListSelection = (selection, savedSelection, fallbackItem) => {
        if (selection && savedSelection && this.content.contains(savedSelection.startContainer) && this.content.contains(savedSelection.endContainer)) {
            selection.removeAllRanges();
            selection.addRange(savedSelection);
            return;
        }

        this.selectListItem(selection, fallbackItem);
    }
    selectListItemStart = (selection, item) => {
    ...
    selectListItem = (selection, item) => {
        if (!selection || !item) { return; }

        const range = document.createRange();
        const firstText = this.getFirstTextNode(item);
        if (firstText) {
    ...
    getFirstTextNode = (node) => {
        ...
        for (let i = 0; i < node.childNodes.length; i++) {
            ...
            if (child.nodeName === 'UL' || child.nodeName === 'OL') { continue; }
            const text = this.getFirstTextNode(child);
            if (text) { return text; }
        }
...
class RTNativeTableDialog {
    ...
    tableTab = () => {
        ...
        if (selection !== null) {
            ...
            const activeElement = this.getCurrentCell(selection);
    ...
    addRowFromLastCell = () => {
        const selection = this.Utilities.getSelection();
        if (selection === null || !selection.rangeCount) { return false; }

        const activeCell = this.getCurrentCell(selection);
        if (!activeCell) { return false; }

        const currentRow = activeCell.parentElement;
        const tbody = currentRow ? currentRow.parentElement : null;
        const table = tbody ? tbody.closest('table') : null;

        if (!currentRow || !tbody || !table) { return false; }
        if (currentRow.nextElementSibling) { return false; }
        if (activeCell.nextElementSibling) { return false; }

        const columns = Math.max(this.getColumns(table), currentRow.cells.length, 1);
    ...
    getCurrentCell = (selection) => {
        ...
        return cell && this.content.contains(cell) ? cell : null;
    }
    selectCell = (cell) => {
        if (!cell) { return; }
...
class RTNativeCodeBlockDialog {
    ...
    openCodeBlockDialogForElement = (pre, range) => {
        if (!pre || !this.content.contains(pre)) { return; }

        this.resetCodeBlockDialog();
        this.savedSelection = range ? range.cloneRange() : null;
        ...
        if (classes != null) {
            classes.value = Array.from(code.classList).join(' ');
        }

        this.codeSelection = range ? range.cloneRange() : null;
        this.code = code;
...
class RTNativeBlockQuoteDialog {
    constructor(shadowRoot, content, utilities) {
        this.shadowRoot = shadowRoot;
        this.content = content;
        this.Utilities = utilities;
    ...
    openBlockQuoteDialogForElement = (blockquote, range) => {
        if (!blockquote || !this.content.contains(blockquote)) { return; }
        ...
        quote.value = blockquote.textContent;

        if (blockquote.cite != null) {
            cite.value = blockquote.cite;
        }
        if (classes != null) {
            classes.value = Array.from(blockquote.classList).join(' ');
        }

        this.quoteSelection = range ? range.cloneRange() : null;
...
class RTNativeUploadImageDialog {
    constructor(shadowRoot, content, utilities) {
        this.shadowRoot = shadowRoot;
        this.content = content;
        this.Utilities = utilities;
...
window.RTNative_Instances = {};

window.RTNative_Initialize = (id, shadow_id, toolbar_id, styles, html, objectReference) => {
    try {
        if (RTNative_Instances[id] == null) {
            RTNative_Instances[id] = new RTNative(id, shadow_id, toolbar_id, styles, objectReference);
            RTNative_Instances[id].loadHtml(html);
        }
    }
    catch (ex) {
        console.log(ex)
...
window.RTNative_Method = (methodName, id, param) => {
    try {
        const editorInstance = RTNative_Instances[id];
...
// ============================================================================
// SVG Icons  (source: RTNativeIcons.cs)
// ============================================================================
const _RTB_ICONS = {
    ...
    InlineCode:          "M544-400 440-504 240-304l104 104 200-200Zm-47-161 104 104 199-199-104-104-199 199Zm-84-28 216 216-229 229q-24 24-56 24t-56-24l-2-2-26 26H60l126-126-2-2q-24-24-24-56t24-56l229-229Zm0 0 227-227q24-24 56-24t56 24l104 104q24 24 24 56t-24 56L629-373 413-589Z",
...
// ============================================================================
// Shadow DOM CSS — injected into the shadow root by RTNative.
...
class RichTextBox extends HTMLElement {
    ...
    static _defaultVisibility(on = true) {
        return {
            // Dropdowns
            font: on, size: on, format: on, insert: on, textStylesDivider: on,
            // Text format
            bold: on, italic: on, underline: on, strikethrough: on,
            subscript: on, superscript: on, inlineCode: on, formatDivider: on,
            // Color
            ...
            // View
            fileMenu: on, saveHtml: on, htmlView: on, preview: on, statusBarToggle: on, search: on, help: on, viewUtilDivider: on,
...
`;
    ...
    attributeChangedCallback(name, oldVal, newVal) {
        ...
        if (name === 'value' && this._initialized) {
            this._value = newVal || '';
            RTNative_Method('loadView', this._uid, newVal);
        }
        ...
        if (name === 'placeholder') {
            const inst = RTNative_Instances?.[this._uid];
            if (inst?.content) {
    ...
    disconnectedCallback() {
        if (window.RTNative_Instances?.[this._uid]) {
            delete RTNative_Instances[this._uid];
        }
    }
    ...
    getValue() {
        return this._initialized
            ? (RTNative_Method('html', this._uid) || this._value || '')
            : (this._value || '');
    }
    ...
    getPlainText() {
        return this._initialized
            ? (RTNative_Method('plaintext', this._uid) || '')
            : '';
    }
    ...
    setValue(html) {
        ...
        this._value = trimmed;
        if (this._initialized) RTNative_Method('loadView', this._uid, trimmed);
    }
    ...
    /**
     * Adds a custom toolbar button. Replaces an existing button with the same id.
     *
     * @param {object} button
     * @param {string} button.id - Unique button id.
     * @param {string} button.title - Tooltip and accessible label.
     * @param {string} button.svg - SVG markup for the button icon.
     * @param {Function|string} button.onClick - Callback or window method name.
     */
    addCustomButton(button) {
        if (!button || !button.id) return;
        this._customButtons = [
            ...this._customButtons.filter(b => b.id !== button.id),
            button
        ];
        this._refreshToolbar();
    }

    /** Replaces all custom toolbar buttons. */
    setCustomButtons(buttons = []) {
        this._customButtons = Array.isArray(buttons)
            ? buttons.filter(button => button && button.id)
            : [];
        this._refreshToolbar();
    }

    /** Removes one custom toolbar button by id. */
    removeCustomButton(id) {
        this._customButtons = this._customButtons.filter(button => button.id !== id);
        this._refreshToolbar();
    }
    ...
    /**
     * Transfers the current preview CSS settings to the RTNative
     ...
     */
    configure(options = {}) {
        const { visibility, customButtons, ...styleOptions } = options;
        ...
        if (customButtons) {
            this._customButtons = Array.isArray(customButtons)
                ? customButtons.filter(button => button && button.id)
                : [];
        }
        ...
        if (this._initialized && (visibility || customButtons)) {
            this._reinitialize();
        }
    }
    ...
    _initialize() {
        ...
        if (!window.RTNative_Instances) window.RTNative_Instances = {};

        RTNative_Instances[this._uid] = new RTNative(
            this._uid,
            `${this._uid}_Shadow`,
            `${this._uid}_Toolbar`,
            _RTB_SHADOW_CSS,
        ...
        RTNative_Instances[this._uid]._rtbHostElement = this;
        ...
        if (ariaLabel) {
            RTNative_Instances[this._uid].content.setAttribute('aria-label', ariaLabel);
        }

        // Placeholder
        const placeholder = this.getAttribute('placeholder');
        if (placeholder) {
            RTNative_Instances[this._uid].content.setAttribute('data-placeholder', placeholder);
        }

        // Read-only
        if (this.hasAttribute('readonly')) {
            RTNative_Instances[this._uid].setReadOnly(true);
        }

        // Word-count status bar visibility
        RTNative_Instances[this._uid]._syncWordCountVisibility();

        RTNative_Instances[this._uid].loadHtml(initialValue);
        this._initialized = true;

        // Transfer preview CSS settings to the RTNative instance and
        // apply scoped styles to the editor content area.
        this._syncToInstance();
        this._applyContentStyles();
        this._attachCustomButtonHandlers();
    }
    ...
    _btn(buttonId, icon, title, method, param) {
        const call = param !== undefined
            ? `RTNative_Method('${method}','${this._uid}','${param}')`
            : `RTNative_Method('${method}','${this._uid}')`;
    ...
    _specialBtn(buttonId, icon, title, method) {
        return `<button id="${buttonId}" title="${title}" aria-label="${title}" aria-pressed="false" class="rich-text-box-menu-item-special" onclick="RTNative_Method('${method}','${this._uid}')">${this._svg(icon)}</button>`;
    }
    ...
    _customButton(button) {
        const id       = this._escapeAttr(button.id);
        const title    = this._escapeAttr(button.title || button.label || button.id);
        const svg      = button.svg || '';
        const disabled = button.disabled ? ' disabled' : '';
        return `<button id="${id}" title="${title}" aria-label="${title}" aria-pressed="false" class="rich-text-box-menu-item rich-text-box-custom-button" data-rtb-custom-button="${id}"${disabled}>${svg}</button>`;
    }

    _customButtonsHTML() {
        if (!this._customButtons.length) return '';
        return this._customButtons.map(button => this._customButton(button)).join('\n');
    }

    _resolveCustomButtonHandler(handler) {
        if (typeof handler === 'function') return handler;
        if (typeof handler !== 'string' || !handler.trim()) return null;
        return handler.split('.').reduce((scope, part) => scope?.[part], window);
    }

    _attachCustomButtonHandlers() {
        const inst = RTNative_Instances[this._uid];
        if (!inst?.shadowRoot) return;

        this._customButtons.forEach(button => {
            const el = inst.shadowRoot.querySelector(`[data-rtb-custom-button="${CSS.escape(button.id)}"]`);
            if (!el) return;

            el.addEventListener('click', (event) => {
                if (button.disabled) return;

                const handler = this._resolveCustomButtonHandler(button.onClick || button.method);
                if (typeof handler === 'function') {
                    handler.call(this, event, this, inst);
                }

                this.dispatchEvent(new CustomEvent('custom-button-click', {
                    detail: { id: button.id, button, editor: this },
                    bubbles: true,
                    composed: true,
                }));
            });
    ...
    _fontDropdown() {
        ...
        const items = fonts.map(f =>
            `<a class="rich-text-box-menu-item" style="font-family:${f};cursor:pointer" onclick="RTNative_Method('font','${id}','${f}')">${f}</a>`
        ).join('');
        return `
<div class="rich-text-box-dropdown">
  <button id="blazing-rich-text-font-button" title="Font (Alt+F)" class="rich-text-box-font-button rich-text-box-dropdown-btn"
    onclick="RTNative_Method('dropdown','${id}','blazing-rich-text-font-button-dropdown')">Font</button>
  <div id="blazing-rich-text-font-button-dropdown" class="rich-text-box-dropdown-content rich-text-box-font-content rich-text-box-scroll">
    ...
    _sizeDropdown() {
        ...
        const items = sizes.map(s => {
            ...
            return `<a class="rich-text-box-menu-item" style="${sty}cursor:pointer" onclick="RTNative_Method('size','${id}','${val}')">${s === 'None' ? 'None' : s}</a>`;
        }).join('');
        return `
<div class="rich-text-box-dropdown">
  <button id="blazing-rich-text-size-button" title="Size (Alt+S)" class="rich-text-box-size-button rich-text-box-dropdown-btn"
    onclick="RTNative_Method('dropdown','${id}','blazing-rich-text-size-button-dropdown')">Size</button>
...
<div class="rich-text-box-dropdown">
  <button id="blazing-rich-text-format-button" title="Format (Alt+T)" class="rich-text-box-format-button rich-text-box-dropdown-btn"
    onclick="RTNative_Method('dropdown','${id}','blazing-rich-text-format-button-dropdown')">Format</button>
  <div id="blazing-rich-text-format-button-dropdown" class="rich-text-box-dropdown-content rich-text-box-format-content rich-text-box-scroll">
    <a class="rich-text-box-menu-item" style="cursor:pointer" onclick="RTNative_Method('format','${id}','none')">None</a>
    <a class="rich-text-box-menu-item" style="cursor:pointer" onclick="RTNative_Method('format','${id}','p')">Paragraph</a>
    <a class="rich-text-box-menu-item" style="font-size:34px;cursor:pointer" onclick="RTNative_Method('format','${id}','h1')">Header 1</a>
    <a class="rich-text-box-menu-item" style="font-size:30px;cursor:pointer" onclick="RTNative_Method('format','${id}','h2')">Header 2</a>
    <a class="rich-text-box-menu-item" style="font-size:24px;cursor:pointer" onclick="RTNative_Method('format','${id}','h3')">Header 3</a>
    <a class="rich-text-box-menu-item" style="font-size:20px;cursor:pointer" onclick="RTNative_Method('format','${id}','h4')">Header 4</a>
    <a class="rich-text-box-menu-item" style="font-size:18px;cursor:pointer" onclick="RTNative_Method('format','${id}','h5')">Header 5</a>
    <a class="rich-text-box-menu-item" style="font-size:16px;cursor:pointer" onclick="RTNative_Method('format','${id}','h6')">Header 6</a>
  </div>
</div>`;
    }
    ...
    _insertDropdown() {
        ...
        const groups = [
            ['Status & Priority', [
                ['✅', 'Success'], ['❌', 'Failed'], ['✔', 'Confirmed'], ['✖', 'Rejected'],
                ['☑', 'Checked'], ['☐', 'Unchecked'], ['⭐', 'Favorite'], ['🌟', 'Featured'],
                ['🔴', 'High Priority'], ['🟡', 'Medium Priority'], ['🟢', 'Low Priority'], ['🔵', 'Informational'],
                ['🏆', 'Best'], ['💯', 'Excellent']
            ]],
            ['Productivity & Workflow', [
                ['⏳', 'In Progress'], ['⌛', 'Waiting'], ['🔄', 'Updating'], ['🔁', 'Repeat'],
                ['🚧', 'Work in Progress'], ['🛠', 'Under Construction / Build'], ['📌', 'Pinned'], ['📍', 'Location'],
                ['📖', 'Documentation'], ['📝', 'Notes'], ['📋', 'Checklist'], ['📎', 'Attachment'],
                ['🔍', 'Search'], ['🧠', 'Thinking'], ['📅', 'Schedule'], ['🚀', 'Launch']
            ]],
            ['Social & Emphasis', [
                ['🔥', 'Trending'], ['✨', 'Exciting'], ['❤️', 'Love'], ['👍', 'Approval'],
                ['👀', 'Attention'], ['👉', 'Pointing'], ['🎯', 'Goal'], ['⚡', 'Fast'],
                ['📢', 'Announcement'], ['💡', 'Idea']
            ]],
            ['Technology', [
                ['🚀', 'Deploy'], ['🐛', 'Bug'], ['✨', 'Feature'], ['⚙', 'Settings'],
                ['🔧', 'Configuration / Tools'], ['📦', 'Package'], ['🔒', 'Secure'], ['🔓', 'Public'],
                ['🧪', 'Experimental'], ['🗑', 'Delete'], ['💾', 'Save']
            ]],
            ['Warnings & Errors', [
                ['⚠', 'Warning'], ['🚨', 'Critical Alert'], ['🔥', 'Serious Issue'], ['🛑', 'Stop'],
                ['❗', 'Important Warning'], ['❓', 'Unknown / Question'], ['⁉', 'Clarification Needed']
            ]],
            ['Arrows & Navigation', [
                ['→', 'Right Arrow'], ['←', 'Left Arrow'], ['↑', 'Up Arrow'], ['↓', 'Down Arrow'],
                ['⇒', 'Result'], ['➜', 'Action Arrow'], ['↳', 'Sub-item'], ['⇢', 'Link']
            ]],
            ['Tree & Hierarchy', [
                ['├─', 'Branch Node'], ['└─', 'Last Branch'], ['│', 'Vertical Line'], ['─', 'Horizontal Line'],
                ['╰─', 'Rounded Last Branch'], ['╭─', 'Rounded Top Branch']
            ]]
        ];
        const items = groups.map(([group, symbols]) => `
    <div class="rich-text-box-insert-group">${group}</div>
    ${symbols.map(([symbol, meaning]) => `
      <button type="button" class="rich-text-box-insert-item" onclick="RTNative_Method('insertEmoji','${id}','${this._escapeAttr(symbol)}')">
        <span class="rich-text-box-insert-symbol">${symbol}</span>
        <span class="rich-text-box-insert-meaning">${meaning || symbol}</span>
      </button>`).join('')}`).join('');
        return `
<div class="rich-text-box-dropdown">
  <button id="blazing-rich-text-insert-button" title="Insert (Alt+I)" class="rich-text-box-insert-button rich-text-box-dropdown-btn"
    onclick="RTNative_Method('dropdown','${id}','blazing-rich-text-insert-button-dropdown')">Insert</button>
  <div id="blazing-rich-text-insert-button-dropdown" class="rich-text-box-dropdown-content rich-text-box-insert-content rich-text-box-scroll">
    ...
    _fileDropdown() {
        ...
        const findItem      = v.search
            ? `<a class="rich-text-box-menu-item" style="cursor:pointer" onclick="RTNative_Method('fileMenuAction','${id}','find')">Find<span class="rich-text-box-menu-shortcut">Ctrl+F</span></a>`
            : '';
        const saveItem      = v.saveHtml
            ? `<a class="rich-text-box-menu-item" style="cursor:pointer" onclick="RTNative_Method('fileMenuAction','${id}','save')">Download<span class="rich-text-box-menu-shortcut">Ctrl+Shift+S</span></a>`
            : '';
        const sep1          = (v.search || v.saveHtml) && v.statusBarToggle
            ? `<hr class="rich-text-box-dropdown-divider">`
            : '';
        const statusItem    = v.statusBarToggle
            ? `<a class="rich-text-box-menu-item" style="cursor:pointer" onclick="RTNative_Method('fileMenuAction','${id}','toggleStatusBar')"><span id="blazing-rich-text-file-statusbar-label">Show Status Bar</span><span class="rich-text-box-menu-shortcut">Ctrl+\\</span></a>`
            : '';
        const sep2          = (v.search || v.saveHtml || v.statusBarToggle) && v.help
            ? `<hr class="rich-text-box-dropdown-divider">`
            : '';
        const shortcutsItem = v.help
            ? `<a class="rich-text-box-menu-item" style="cursor:pointer" onclick="RTNative_Method('fileMenuAction','${id}','shortcuts')">Keyboard Shortcuts<span class="rich-text-box-menu-shortcut">Ctrl+/</span></a>`
            : '';
        return `
<div class="rich-text-box-dropdown">
  <button id="blazing-rich-text-file-button" title="File (Alt+L)" class="rich-text-box-file-button rich-text-box-dropdown-btn"
    onclick="RTNative_Method('dropdown','${id}','blazing-rich-text-file-button-dropdown')">File</button>
  <div id="blazing-rich-text-file-button-dropdown" class="rich-text-box-dropdown-content rich-text-box-file-content rich-text-box-scroll">
    ${findItem}${saveItem}${sep1}${statusItem}${sep2}${shortcutsItem}
  </div>
</div>`;
    }
    ...
    _colorSwatches(selectMethod) {
        ...
        return colors.map(c =>
            `<div class="blazing-rich-text-color-option" onclick="RTNative_Method('${selectMethod}','${this._uid}','${c}')" style="background-color:${c};"></div>`
        ).join('');
    }
...
<dialog id="${dialogId}" class="rich-text-box-modal rich-text-box-scroll" aria-modal="true" aria-labelledby="${dialogId}-title">
  <div class="rtb-modal-header">
    ...
    <button class="rich-text-box-modal-close" aria-label="Close dialog" onclick="RTNative_Method('closeDialog','${id}','${dialogId}')"><svg viewBox="0 0 12 12" aria-hidden="true" focusable="false" width="8" height="8" style="display:block"><path d="M2 2L10 10M10 2L2 10" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/></svg></button>
  </div>
  <div class="rich-text-box-color-picker">
    ...
    <div style="text-align:right;">
      <button class="rich-text-box-form-button" onclick="RTNative_Method('closeDialog','${id}','${dialogId}')">Close</button>
      <button id="rich-text-box-ok-button" class="rich-text-box-form-button" onclick="RTNative_Method('${insertMethod}','${id}')">OK</button>
    </div>
  </div>
</dialog>`;
    }
...
<dialog id="${dialogId}" class="rich-text-box-modal rich-text-box-scroll" aria-modal="true" aria-labelledby="${dialogId}-title">
  ...
  <div>
    ...
    _buildToolbarHTML() {
        ...
        const textStylesDividerNeeded = (v.font || v.size || v.format || v.insert) && v.textStylesDivider;
        const formatDividerNeeded     = (v.bold || v.italic || v.underline || v.strikethrough || v.subscript || v.superscript || v.inlineCode) && v.formatDivider;
...
<div id="${id}_Toolbar" class="rich-text-box-tool-bar" role="toolbar" aria-label="Formatting toolbar">
  ...
  ${v.inlineCode  ? this._btn('blazing-rich-text-inline-code-button', 'InlineCode',  'Inline Code (Ctrl+Shift+E)',      'inlinecode')  : ''}
  ...
  ${customButtonsDividerNeeded ? this._divider() : ''}
...
<dialog id="${id}_Preview" class="rich-text-box-modal rich-text-box-scroll rtb-preview-dialog" aria-modal="true" aria-labelledby="${id}_Preview-title">
  <div class="rtb-modal-header">
    ...
    <div style="display:flex;gap:2px;align-items:center;">
      <button id="${id}_PreviewMaxBtn" class="rich-text-box-modal-close" aria-label="Maximize preview" onclick="RTNative_Method('togglePreviewMaximize','${id}')"><svg viewBox="0 0 12 12" aria-hidden="true" focusable="false" width="11" height="11" style="display:block"><path d="M1.5 4.5V1.5H4.5M10.5 7.5V10.5H7.5M1.5 1.5L4.5 4.5M10.5 10.5L7.5 7.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg></button>
      <button class="rich-text-box-modal-close" aria-label="Close preview" onclick="RTNative_Method('closePreview','${id}')"><svg viewBox="0 0 12 12" aria-hidden="true" focusable="false" width="8" height="8" style="display:block"><path d="M2 2L10 10M10 2L2 10" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/></svg></button>
    </div>
  </div>
  <div class="rtb-preview-dialog-body">
    <div id="rich-text-box-preview" class="rtb-preview-window"></div>
    <div style="text-align:right;flex-shrink:0;">
      <button class="rich-text-box-form-button" onclick="RTNative_Method('closePreview','${id}')">Close</button>
    </div>
  </div>
</dialog>`;
    }
...
<dialog id="rich-text-box-help-modal" class="rich-text-box-modal rtb-help-dialog" aria-labelledby="rich-text-box-help-modal-title">
  <div class="rtb-help-sticky-header">
    ...
    <button class="rich-text-box-modal-close" aria-label="Close" onclick="RTNative_Method('closeHelpDialog','${id}')"><svg viewBox="0 0 12 12" aria-hidden="true" focusable="false" width="8" height="8" style="display:block"><path d="M2 2L10 10M10 2L2 10" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/></svg></button>
  </div>
